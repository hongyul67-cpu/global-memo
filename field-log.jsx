...
// 선생님 구글 계정에 배포한 Apps Script 웹앱 주소 (기록을 직접 받아 스프레드시트+드라이브에 저장)
...
export default function App() {
  ...
  async function addPhotos(files) {
    ...
    if (next.length) setForm((f) => ({ ...f, photos: [...f.photos, ...next] }));
  }
  ...
  function save() {
    ...
    if (editingId) {
      // 내용이 바뀌었으니 '제출됨' 표시를 지워 다시 보낼 수 있게 함
      setRecords((rs) => rs.map((r) => (r.id === editingId ? { ...form, id: editingId, sentAt: null } : r)));
    } else {
  ...
  // ---- 선생님께 전송 (Google Apps Script) ----
  async function postRecord(r) {
    const payload = {
      token: SUBMIT_TOKEN, name,
      date: r.date, place: r.place, visit: r.visit,
      keywords: r.keywords || [], facts: r.facts,
      impression: r.impression, difference: r.difference,
      question: r.question, photos: r.photos || [],
    };
  ...
  async function submitUnsent() {
    ...
    if (sentIds.length) {
      ...
      setRecords((rs) => rs.map((r) => (sentIds.includes(r.id) ? { ...r, sentAt: now } : r)));
    }
    setSending(false);
    setSendResult(
      fail === 0
        ? { ok: true, msg: `제출 완료! 기록 ${sentIds.length}건을 PDF로 만들어 선생님께 보냈어요.` }
  ...
  const unsentCount = records.filter((r) => !r.sentAt).length;
      ...
      <div className="mx-auto w-full max-w-md px-4 pb-28">
        ...
        {screen === "list" && (
          <div className="pt-8">
            ...
            <button type="button" onClick={submitUnsent} disabled={sending || records.length === 0}
              className="mt-3 w-full flex items-center justify-between rounded-xl px-4 py-3 no-print"
              style={{ background: C.amber, color: "#fff", opacity: (sending || !records.length) ? 0.6 : 1 }}>
              <span className="text-sm font-semibold">
                {sending ? "선생님께 보내는 중…" : `저녁에 · 안 보낸 기록 선생님께 제출하기${unsentCount ? ` (${unsentCount})` : ""}`}
              </span>
              <span className="text-lg leading-none">{sending ? "⏳" : "→"}</span>
            </button>
            {sendResult && (
              <div className="mt-2 rounded-xl px-3 py-2 text-xs leading-relaxed no-print"
                style={{ background: sendResult.ok ? C.blueSoft : C.amberSoft, color: sendResult.ok ? C.blue : C.amber, border: `1px solid ${sendResult.ok ? C.blueLine : C.amberLine}` }}>
                {sendResult.msg}
              </div>
            )}
                  ...
                  <button type="button" key={r.id} onClick={() => openEdit(r)} className="block w-full text-left rounded-2xl p-4 transition-shadow"
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-xs flex items-center gap-1.5">
                        {r.sentAt
                          ? <span style={{ color: "#2E7D46", fontWeight: 700 }}>✓ 제출됨</span>
                          : <span style={{ color: C.amber, fontWeight: 700 }}>미제출</span>}
                        <span style={{ color: C.steel }}>{r.photos.length}장</span>
                      </div>
                    </div>
        ...
        {screen === "export" && (
          <div className="pt-6">
            <div className="no-print">
              <div className="flex items-center justify-between">
                <button type="button" onClick={() => setScreen("list")} className="text-sm font-medium" style={{ color: C.steel }}>‹ 목록</button>
                <div className="text-sm font-semibold">PDF로 저장 (보고서)</div>
                <div className="w-10" />
              </div>
              <p className="mt-3 text-sm leading-relaxed" style={{ color: C.sub }}>
                포함할 날짜를 고르고 <b>PDF로 저장</b>을 누르면 인쇄 화면이 열려요. 거기서 ‘대상’을 <b>PDF로 저장</b>으로 바꾸면 보고서 PDF가 만들어져요. (제출은 목록 화면의 <b>선생님께 제출하기</b>로 하면 됩니다.)
              </p>
